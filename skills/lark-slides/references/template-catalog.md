...
> **机器优先路径。** 优先运行 [`../scripts/template-tool.py`](../scripts/template-tool.py) 的 `search` 做低成本路由；锁定模板后运行 `summarize` 获取主题和布局摘要；只有需要具体布局骨架时才运行 `extract` 裁切目标页型 XML。`template-index.json` 是脚本缓存/轻量路由索引，不是默认阅读入口。

> **降级路径。** 如果 `template-tool.py` 不可用，使用本文按场景确定候选模板和页型，但不要因为脚本不可用就直接阅读全文模板 XML。
...
1. 先运行 `python3 skills/lark-slides/scripts/template-tool.py search --query "<主题>" --limit 3`，根据用户描述的 **场景、风格、色调** 做初筛
...
5. 优先运行 `template-tool.py summarize` 查看 `<theme>` / 页型摘要；只有需要具体布局骨架时，再运行 `template-tool.py extract`
